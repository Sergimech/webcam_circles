...
int main(int argc, char *argv[]) 
{
	...
	//Read image and check it. Blocking call up to a new image arrives from camera.
        ...
        //Clear previous circles to avoid overlays
        circles.clear();
        ...
        for(unsigned int ii = 0; ii < circles.size(); ii++ )
        {
            if ( circles[ii][0] != -1 )
            {
                    ...
                    cv::circle(image, center, 5, cv::Scalar(255,0,0), -1, 8, 0 );// Circle center in blue
                    cv::circle(image, center, radius, cv::Scalar(0,0,255), 2, 3, 0 );// Circle perimeter in red
            }
        }      
    ...
    //********************************************************************
        ...
        //Show image
        cv::imshow("Find circles Show", image);
